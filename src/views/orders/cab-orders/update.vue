<template>
  <el-main>
    ...
    >
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="Admin" class="custom-col" prop="creator_id">
            <el-input
              placeholder="Admin"
              :value="`${cabOrderForm.creator.name} ${cabOrderForm.creator.email}`"
        ...
        <el-col :span="8">
          <el-form-item label="Haydovchi" class="custom-col">
            <el-input
              ...
              :value="`${cabOrderForm.driver.name} ${cabOrderForm.driver.email}`"
      ...
      <el-button type="success" @click="updateCabOrder()">Saqlash</el-button>
  ...
  methods: {
    updateCabOrder() {
      this.$refs.cabOrderForm.validate((valid) => {
        if (valid) {
          this.$store
            .dispatch("cab-order/updateCabOrder", this.cabOrderForm)
            .then(() => {
              this.$router.go(-1);
              this.$message({
                showClose: true,
    ...
    console.log(this.cabOrderForm);
      this.driverDialog = false;
    },
    getCabOrder() {
      this.$store
        .dispatch("cab-order/getCabOrder", this.$route.params.id)
        .then((response) => {
          this.cabOrderForm = response.data;
        })
        .catch((error) => {
          this.$message({
            showClose: true,
